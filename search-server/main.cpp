...
public:
    ...
    template <typename DocumentPredicate>
       ...
       optional<vector<Document>> FindTopDocuments(const string& raw_query, DocumentStatus filter_status) const {
    ...
    template <typename DocumentPredicate>
       optional<vector<Document>> FindTopDocuments(const char* raw_query, DocumentPredicate document_predicate) const {
           return FindTopDocuments(static_cast<string>(raw_query), document_predicate);
       }

       optional<vector<Document>> FindTopDocuments(const char* raw_query, DocumentStatus filter_status) const {
           return FindTopDocuments(static_cast<string>(raw_query), filter_status);
       }

       optional<vector<Document>> FindTopDocuments(const char* raw_query) const {
       ...
       optional<tuple<vector<string>, DocumentStatus>> MatchDocument(const char* raw_query, int document_id) const {
...
void TestMatchDocument(){
    ...
    ASSERT(result1.has_value() && get<0>(result1.value()).size() == 0);
...
template<typename T>
optional<T> Back(T abc, int x, int y){
    for(auto& el: abc){
        el = x+y;
    }
    return abc;
...
int main() {
    ...
    vector abc(2, 0);
    auto a = Back(abc, 2, 10);
    cout << a.value() << endl;

    SearchServer search_server("и в на"s);
    // Явно игнорируем результат метода AddDocument, чтобы избежать предупреждения
    // о неиспользуемом результате его вызова
    (void) search_server.AddDocument(0, "белый кот и модный ошейник"s, DocumentStatus::ACTUAL, {10});
    (void) search_server.AddDocument(1, "пушистый кот пушистый хвост"s, DocumentStatus::ACTUAL, {12});
    (void) search_server.AddDocument(2, "ухоженный пёс выразительные глаза"s, DocumentStatus::ACTUAL, {17});
    ...
    if (const auto documents = search_server.FindTopDocuments("пушистый ухоженный кот -ошейник"s)) {
