...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        map<int, double> document_to_relevance = MatchDocuments (query);
    ...
    double ComputeWordInverseDocumentFreq(const string& word) const {
     ...
     map<int, double> MatchDocuments(const Query& query) const{
         ...
         for (const auto& plus_word : query.plus_words) {
              if(word_to_document_freqs_.count(plus_word)){
                      ...
                      for(const auto& [id, freqs] : word_to_document_freqs_.at(plus_word)){
                          document_to_relevance[id] += freqs * inverse_document_freq;
                      }
              }
         }
